 ...
 */
class pdf_extended extends ModeleExpenseReport
{
     /**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "extended";
		$this->description = $langs->trans('PDFExtendedExpenseReports');
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		*/
		...
		$pdf->SetFont('', '', $default_font_size - 1);
   		...
   		$pdf->SetXY($posx, $posy);
   		$pdf->SetTextColor(0, 0, 60);
   		$pdf->MultiCell($this->page_largeur - $this->marge_droite - $posx, 3, $outputlangs->transnoentities("Ref")." : ".$object->ref . $fileVersion, '', 'R');
        ...
        if ( !empty( $arrayoffiles ) ) { 
            foreach ($arrayoffiles as $file) {
                ...
                if (preg_match('/\.(jpg|png|jpeg)$/', $file['name'])) {
                    ...
                    $filename = $conf->file->dol_document_root['alt0'] . '/dolifraispro/documents/expensereport/icone-image-cassee.png';
                    list($width, $height, $type) = getimagesize($filename);
